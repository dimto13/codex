...
//! | `AREN_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static AREN_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
pub(crate) fn set_theme_override(
    ...
    let codex_home_set_ok = AREN_HOME.set(codex_home.clone()).is_ok();
...
pub(crate) fn validate_theme_name(name: Option<&str>, codex_home: Option<&Path>) -> Option<String> {
    ...
    let custom_theme_path_display = codex_home
        ...
        .unwrap_or_else(|| format!("$AREN_HOME/themes/{name}.tmTheme"));
...
fn resolve_theme_with_override(name: Option<&str>, codex_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {AREN_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = codex_home
...
fn build_default_theme() -> Theme {
    ...
    let codex_home = AREN_HOME.get().and_then(|codex_home| codex_home.as_deref());
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = AREN_HOME.get()
...
/// `.tmTheme` file under `{AREN_HOME}/themes/`.
