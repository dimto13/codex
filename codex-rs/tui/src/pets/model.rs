...
//! in-memory `Pet`: built-in catalog ids, custom pet ids under AREN_HOME,
...
impl Pet {
    ...
    /// has already been materialized into AREN_HOME; if callers skip the
...
fn load_builtin_pet(pet: catalog::BuiltinPet, codex_home: Option<&Path>) -> Result<Pet> {
    let codex_home = codex_home.context("AREN_HOME is not available")?;
