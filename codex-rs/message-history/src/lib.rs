...
//!
//! The history is stored at `~/.aren/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.aren`.
...
pub async fn append_entry(
    ...
    // Resolve `~/.aren/history.jsonl` and ensure the parent directory exists.
