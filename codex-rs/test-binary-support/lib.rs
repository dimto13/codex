    ...
    match classify(exe_name, argv1.as_deref().and_then(|arg| arg.to_str())) {
        ...
        TestBinaryDispatchMode::InstallAliases => {
            let codex_home = match tempfile::Builder::new().prefix(codex_home_prefix).tempdir() {
                ...
                Err(error) => panic!("failed to create test AREN_HOME: {error}"),
            };
            let previous_codex_home = std::env::var_os("AREN_HOME");
            // Safety: this runs from a test ctor before test threads begin.
            unsafe {
                std::env::set_var("AREN_HOME", codex_home.path());
            }
            ...
            match previous_codex_home.as_ref() {
                Some(value) => unsafe {
                    std::env::set_var("AREN_HOME", value);
                },
                None => unsafe {
                    std::env::remove_var("AREN_HOME");
                },
            }
