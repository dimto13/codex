...
impl OAuthStoreLock {
    pub(super) fn acquire(store: OAuthStore) -> Result<Self, OAuthStoreLockFailure> {
        ...
        // authority. Those stores are AREN_HOME-backed today: if AREN_HOME is unset they use
        // the default home (`~/.aren`), and if an embedder has no local home/filesystem authority
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve AREN_HOME for MCP OAuth {store} aggregate-store lock")]
    CodexHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
