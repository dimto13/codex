...
//! - sensitive subpaths such as `.git`, `.agents`, and `.aren` remain
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.aren`.
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let logical_codex = logical_home.join(".aren");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".aren"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_codex = path_to_string(&temp_dir.path().join(".aren"));
