...
///
/// 1.  Load `.env` values from `~/.aren/.env` before creating any threads.
...
fn prepare_path_entry_for_codex_aliases(
    ...
    // Use a AREN_HOME-scoped temp root to avoid cluttering the top-level directory.
