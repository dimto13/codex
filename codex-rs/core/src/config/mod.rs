...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.aren/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// file: AREN_HOME/.credentials.json
    ...
    /// Directory containing all Codex state (defaults to `~/.aren` but can be
    /// overridden by the `AREN_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Directory where Codex stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Codex writes log files (defaults to `$AREN_HOME/log`).
    pub log_dir: PathBuf,
    ...
    /// Settings that govern if and what will be written to `~/.aren/history.jsonl`.
    pub history: History,
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .aren/ folders.
...
/// Patch `AREN_HOME/config.toml` project state to set trust level.
/// Use with caution.
