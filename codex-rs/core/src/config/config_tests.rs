...
-> std::io::Result<()> {
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".aren"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".aren".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".aren"), cwd.as_path()),
            "expected .aren carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".aren"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".aren"] {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    ...
    let project_dot_codex =
        AbsolutePathBuf::resolve_path_against_base("project/.aren", codex_home.path());
...
"#,
    ...
    let project_config_dir = workspace.path().join(".aren");
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".aren").join("agents");
