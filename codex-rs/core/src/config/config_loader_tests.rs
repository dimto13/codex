...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when AREN_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when AREN_HOME/config.toml does not exist");
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".aren")).await?;
    tokio::fs::create_dir_all(project_root.join(".aren")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".aren").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".aren").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    assert_eq!(project_layers[0].as_path(), nested.join(".aren").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".aren").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".aren")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".aren")).await?;
    ...
    .await?;
    write_project_hook_config(
        &repo_child.join(".aren"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".aren"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".aren"),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    .await?;
    write_project_hook_config(
        &project_root.join(".aren"),
        /*foo*/ None,
        "echo project root hook",
    )
    .await?;
    write_project_hook_config(&nested.join(".aren"), /*foo*/ None, "echo nested hook").await?;
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".aren"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
"#;
    tokio::fs::write(project_root.join(".aren").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".aren").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".aren").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(nested.join(".aren").join("child.txt"), "child instructions").await?;
...
async fn project_layer_is_added_when_dot_codex_exists_without_config_toml() -> std::io::Result<()> {
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_codex_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".aren"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
async fn codex_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    ...
    let codex_home = home_dir.join(".aren");
    tokio::fs::create_dir_all(&codex_home).await?;
    tokio::fs::write(
        codex_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
async fn codex_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".aren");
    let nested_dot_codex = nested.join(".aren");
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let dot_codex = project_root.join(".aren");
    ...
    // `.aren` folder. The malformed profile value below would fail typed path
...
"#,
    ...
    // still use the project-local `.aren` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".aren")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".aren").join(CONFIG_TOML_FILE), "foo =").await?;
