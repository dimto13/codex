...
/// This test writes to a fallback credentials file in AREN_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire AREN_HOME through the code.
#[test]
#[serial(codex_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 3: seed an isolated AREN_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _codex_home_guard = EnvVarGuard::set("AREN_HOME", temp_home.path().as_os_str());
