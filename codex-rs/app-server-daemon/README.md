...
$HOME/.aren/packages/standalone/current/codex app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `AREN_HOME/app-server-daemon/`, starts app-server as a
...
the standalone managed binary under `AREN_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `AREN_HOME/packages/standalone/current/codex` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `AREN_HOME/packages/standalone/current/codex` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
All mutating lifecycle commands are serialized per `AREN_HOME`, so a concurrent
...
The daemon stores its local state under `AREN_HOME/app-server-daemon/`:

- `settings.json` for persisted launch settings
- `aren.pid` for the app-server process record
- `aren-updater.pid` for the pid-backed standalone updater loop
