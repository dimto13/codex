...
/// - user      `${AREN_HOME}/config.toml`
/// - profile   `${AREN_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.aren/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.aren/config.toml` (loaded but disabled when untrusted)
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".aren"))
    }
}
...
async fn load_project_layers(
    ...
    for dir in dirs {
        let dot_codex_abs = dir.join(".aren");
