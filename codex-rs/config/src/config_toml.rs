...
/// Base config deserialized from ~/.aren/config.toml.
...
pub struct ConfigToml {
    ...
    /// Defaults to `$AREN_SQLITE_HOME` when set. Otherwise uses `$AREN_HOME`.
    ...
    /// directories for `.aren` folders. Defaults to [".git"] when unset.
    #[serde(default)]
