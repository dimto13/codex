...
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in AREN_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in AREN_HOME.
    Auto,
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$AREN_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against AREN_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
