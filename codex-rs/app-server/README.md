...
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$AREN_HOME/app-server-control/aren.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
opens exactly one raw stream connection to `$AREN_HOME/app-server-control/aren.sock`
...
- `memory/reset` — experimental; clear the current `AREN_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.aren/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.aren/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.aren/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.aren/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.aren/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.aren/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.aren/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
