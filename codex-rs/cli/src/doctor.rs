...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("AREN_HOME: {}", config.codex_home.display()));
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "AREN_HOME", &config.codex_home);
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match codex_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "AREN_HOME was resolved without config",
        )
        .detail(format!("AREN_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "AREN_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
