...
const PROTECTED_METADATA_CODEX_PATH_NAME: &str = ".aren";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".aren");
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            // Example: if `<root>/.aren -> <root>/decoy`, bwrap must still see
            // `<root>/.aren`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.aren` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    // Keep top-level project metadata under .aren read-only to the agent by
...
mod tests {
    ...
    fn writable_roots_proactively_protect_missing_dot_codex() {
        ...
        let expected_dot_codex = expected_root.join(".aren");
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry {
                    path: FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".aren".into())),
                    },
                    access: FileSystemAccessMode::Read,
                },
            ])
    ...
    fn writable_roots_skip_default_dot_codex_when_explicit_user_rule_exists() {
        ...
        .expect("absolute canonical root");
        let explicit_dot_codex = expected_root.join(".aren");
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".aren".to_string()),
            "explicit .aren rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_codex),
            "explicit .aren rule should win over the default protected carveout"
        );
        assert!(
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_codex_config = cwd.path().join(".aren").join("config.toml");
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".aren/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let codex_dir = real_root.join(".aren");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&codex_dir).expect("create .aren");
        ...
        let expected_codex = link_root.join(".aren");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_codex = expected_root.join(".aren");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let dot_codex = root.join(".aren");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_codex).expect("create .aren symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let codex_dir = real_tmpdir.join(".aren");
