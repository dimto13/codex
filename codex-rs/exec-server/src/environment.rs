...
impl EnvironmentManager {
    ...
    /// Builds a manager from `AREN_HOME` and local runtime paths used when
    /// creating local filesystem helpers.
    ///
    /// If `AREN_HOME/environments.toml` is present, it defines the configured
    ...
    /// reading user config files from `AREN_HOME`.
