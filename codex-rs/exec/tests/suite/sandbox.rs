...
async fn sandbox_blocks_first_time_dot_codex_creation() {
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .aren && echo 'sandbox_mode = \"danger-full-access\"' > .aren/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .aren");

    let status = child.wait().await.expect("should wait for .aren command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .aren creation: {status:?}"
    );
