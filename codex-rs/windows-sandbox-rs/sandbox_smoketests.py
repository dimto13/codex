...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"AREN_HOME": str(proxy_home)},
            )
    ...
    # 34. WS: policy tamper (.aren artifacts) denied
    codex_home = Path(os.environ["USERPROFILE"]) / ".aren"
    ...
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .aren\\policy.json"], WS_ROOT)
    add("WS: .aren cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .aren policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
