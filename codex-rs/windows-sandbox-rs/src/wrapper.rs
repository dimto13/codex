...
const AREN_HOME_FLAG: &str = "--codex-home";
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            AREN_HOME_FLAG => codex_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?)),
            COMMAND_CWD_FLAG => {
    ...
    let codex_home = codex_home.ok_or_else(|| anyhow!("missing required {AREN_HOME_FLAG}"))?;
    if !codex_home.is_absolute() {
        bail!(
            "{AREN_HOME_FLAG} must be absolute: {}",
            codex_home.display()
        );
    }
