...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, codex_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to AREN_HOME or anything under AREN_HOME/.sandbox,
    // AREN_HOME/.sandbox-bin, or AREN_HOME/.sandbox-secrets. These locations contain sandbox
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_codex).expect("canonical extra .aren"),
                explicit_deny,
            ]
            .into_iter()
