...
Set a stable path to the skill CLI (default `AREN_HOME` is `~/.aren`):

```
export AREN_HOME="${AREN_HOME:-$HOME/.aren}"
export IMAGE_GEN="$AREN_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$AREN_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
