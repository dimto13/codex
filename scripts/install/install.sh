...
AREN_HOME_DIR="${AREN_HOME:-$HOME/.aren}"
STANDALONE_ROOT="$AREN_HOME_DIR/packages/standalone"
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    find "$BIN_DIR" -mindepth 1 -maxdepth 1 -name '.aren.*' -exec rm -f {} +
  fi
}
